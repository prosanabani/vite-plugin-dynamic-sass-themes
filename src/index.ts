 ...
 */
export default function dynamicSassThemePlugin(
  options: DynamicSassThemePluginOptions = {}
): Plugin {
  const {
    themesDir = "src/themes", // Default themes directory
  ...
  const compileAndSaveTheme = (
    themeScssPath: string,
    ...
    try {
      ...
      if (log) {
        console.log(
          `[vite-plugin-dynamic-sass-themes] Compiled ${themeScssPath} to ${outputCssPath}`
        );
      }
      ...
      console.error(
        `[vite-plugin-dynamic-sass-themes] Error compiling ${themeScssPath}:`,
        error
      );
      return null;
    }
    ...
    generateBundle() {
      ...
      themeNames.forEach((themeName) => {
        ...
        if (fs.existsSync(themeScssPath)) {
          ...
          console.warn(
            `[vite-plugin-dynamic-sass-themes] No theme.scss found in ${themeDir}`
          );
        }
      });
    },
